...
const EvidenceDashboard = ({ practitioner }) => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      console.error(e);
      setStatus({ type: 'error', message: 'Error de red: No se pudieron cargar los datos.' });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {view === 'folder' && selectedCase && (
        <div className="space-y-6">
          ...
          {status.type === 'error' && view === 'list' && (
            <div className="p-8 text-center bg-red-50 rounded-[2rem] border border-red-100">
              <p className="text-red-600 font-bold mb-4">{status.message}</p>
              <button onClick={loadData} className="px-6 py-2 bg-slate-900 text-white rounded-xl text-xs font-black uppercase tracking-widest">Reintentar Conexión</button>
            </div>
          )}
